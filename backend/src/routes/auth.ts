...
import { signAccessJwt, signRefreshJwt, verifyRefreshJwt } from "../services/token.service";
...
auth.post("/auth/login", authLimiter, validate(LoginSchema), async (req, res, next) => {
  try {
    ...
    if (user.role === "employee" && user.employee && user.employee.active === false) {
      return res.status(403).json({ error: "employee_disabled" });
    }
    ...
    const accessToken = await signAccessJwt(uid, { role: user.role });
...
auth.post("/auth/refresh", authLimiter, async (req, res, next) => {
  try {
    ...
    const accessToken = await signAccessJwt(String(userId), { role: user.role });
...
auth.post("/auth/logout", authLimiter, (req, res) => {
